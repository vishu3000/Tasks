...
import ModifyList from "@/Components/Modify/ModifyList";
import CreateNewList from "@/Components/CreateNew/CreateNewList";

export default function page({ searchParams }) {
  const showModifyModal = searchParams?.modifyList;
  const showCreateNewModal = searchParams?.createNewList;
 
  return (
    <>
      {showModifyModal && <ModifyList />}
      {showCreateNewModal && <CreateNewList />}
      <div className={styles.parentContainer}>
        {/* Buttons Container */}
        <Buttons
          path1="/list/?modifyList=true"
          path2="/list/?createNewList=true"
        />
